...
export function WaitlistForm({ variant = "light", disabled = false }: WaitlistFormProps) {
  ...
  const StrikethroughField = ({ children }: { children: React.ReactNode }) => (
    <div className="relative">
      {children}
      {disabled && (
        <div className="absolute inset-0 flex items-center pointer-events-none" aria-hidden="true">
          <div className={`w-full h-px ${isDark ? "bg-white/30" : "bg-border"}`} />
        </div>
      )}
    </div>
  )
  ...
  return (
    <form onSubmit={handleSubmit} className={`w-full max-w-lg mx-auto ${disabled ? "opacity-45 pointer-events-none select-none" : ""}`} aria-disabled={disabled || undefined}>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-2 mt-1">
          {insuranceOptions.map((opt) => (
            <StrikethroughField key={opt.id}>
              <label
                className={`flex items-center gap-3 rounded-sm transition-all duration-200 px-4 py-3 ${
